...
#include "ofproto/docker_config.h"
#include "ofproto/srv_task.h"
...
void compose_output_action(struct xlate_ctx *ctx, ofp_port_t ofp_port)
{
                    ...
                    ofp_port_t port, uint16_t max_len, bool may_packet_in)
{
    ...
    case OFPP_CONTROLLER:
        ...
        break;
    case OFPP_MAX_1:
    //Example: In-House controller simply output action
    ...
    case OFPP_MAX_2:
        //Example: In-House controller header modification and output action
		if(ctx->xin->flow.in_port.ofp_port==1&&ctx->xin->flow.dl_type==0x0800/*ip packet*/) {
			ctx->xin->flow.nw_src = 0xC0A80201; //modify source ip to 192.168.2.1
			ctx->xin->flow.nw_src = 0xC0A80202; //modify destination ip to 192.168.2.2
			compose_output_action(ctx, 2);
		}
		if(ctx->xin->flow.in_port.ofp_port==2&&ctx->xin->flow.dl_type==0x0800/*ip packet*/) {
			ctx->xin->flow.nw_src = 0xC0A80302; //modify source ip to 192.168.3.21
			ctx->xin->flow.nw_src = 0xC0A80301; //modify destination ip to 192.168.3.1
			compose_output_action(ctx, 1);
		}
		break;
    ...
    case OFPP_MAX_3:
    	...
    	flow1_actions.ofpact.type = OFPACT_OUTPUT;
    	...
    	break;
    case OFPP_MAX_4:
    //Example: In-House controller slow path action
    	//The packet content is accessible, packet forwarding speed is slow
		ctx->xout->slow |= SLOW_CONTROLLER;
		if (!ctx->xin->packet) {
			return;
		}
		pPacket = srv_msg_alloc(ctx->xin->packet->size_);
		memcpy(pPacket, ctx->xin->packet, ctx->xin->packet->size_);
		//packet is sent to TID_TEST_RECVER thread, received in test_recver_entry() function in docker_test.c
		if(srv_msg_snd(TID_OFPROTO,TID_TEST_RECVER,SRV_MSG_NORMAL_PRIO,1,pPacket)!=SRV_OK){
			sprintf(str+strlen(str),"--=== msg_send failed ===--\n");
