...
enum ofperr
handle_packet_out_docker(char* name,  const struct ofp_header *oh)
{
	struct ofproto *p = ofproto_lookup(name);
...
exit_free_ofpacts:
    ofpbuf_uninit(&ofpacts);//--=== entering in a while
exit:
    return error;
}
